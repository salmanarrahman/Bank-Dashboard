...
$userData = fileOps::getSingleUser($data);
...
if ($_SERVER["REQUEST_METHOD"] === 'POST') {
    ...
    if (empty($_POST['amount'])) {
        $errors['amount'] = 'Please provide a amount';
    } else {
        $amount = helpers::sanitize($_POST['amount']);
    }

    if (empty($_POST['amount'])) {
        $errors['amount'] = 'Please provide a amount';
    }else if ($_POST['amount'] > $balance || $_POST['amount'] < 0) {
    ...
    if (empty($errors)) {
        ...
        if ($balance > 0){
            $receiverName = fileOps::returnName($email,fileOps::getAllUsers());
            ...
            fileOps::addUserBalance($email,$amount,"transfer");
            fileOps::withdrawUserBalance($data,$amount);
            fileOps::addTransaction($trans);
